...
//        continue; // skips directly to the next image without processing what comes beneath
        ...
        // extract 2D keypoints from current image: SHITOMASI, HARRIS, FAST, BRISK, ORB, AKAZE, SIFT
        vector<cv::KeyPoint> keypoints; // create empty feature list for current image # recommend ORB/BRIEF
...
//             string selectorType = "SEL_NN"; // choose nearest neighbors (NN)
            ...
            for (auto it1 = (dataBuffer.end() - 1)->bbMatches.begin();
                    ...
                    prevBB->lidarPoints.size() > 0) // only compute TTC if we have Lidar points
                {
                    ...
                    if (test_CAM){
                        computeTTCCameraCAM((dataBuffer.end() - 2)->keypoints, (dataBuffer.end() - 1)->keypoints,
                    ...
                    // test performance
                    cout<<"detector: "<<detectorType<<" with descripter: "<<descriptorType<< " lidar TTC(s): "<<ttcLidar<<" camera TTC(s): "<< ttcCamera<<endl;
